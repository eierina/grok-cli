...
- Consult [GROK.md](https://github.com/google-gemini/gemini-cli/blob/main/GROK.md) (typically found in the project root) for specific instructions related to AI-assisted development, including conventions for React, comments, and Git usage.
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect gemini-cli due to automatic exclusion. Use `.grok/.env` files for gemini-cli specific debug settings.
...
Container-based sandboxing mounts the project directory (and system temp directory) with read-write access and is started/stopped/removed automatically as you start/stop Gemini CLI. Files created within the sandbox should be automatically mapped to your user/group on host machine. You can easily specify additional mounts, ports, or environment variables by setting `SANDBOX_{MOUNTS,PORTS,ENV}` as needed. You can also fully customize the sandbox for your projects by creating the files `.grok/sandbox.Dockerfile` and/or `.grok/sandbox.bashrc` under your project settings directory (`.grok`) and running `gemini` with `BUILD_SANDBOX=1` to trigger building of your custom sandbox.
