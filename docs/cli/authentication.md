...
You can create a **`.grok/.env`** file in your project directory or in your home directory. Creating a plain **`.env`** file also works, but `.grok/.env` is recommended to keep Gemini variables isolated from other tools.

**Important:** Some environment variables (like `DEBUG` and `DEBUG_MODE`) are automatically excluded from project `.env` files to prevent interference with gemini-cli behavior. Use `.grok/.env` files for gemini-cli specific variables.
...
```bash
mkdir -p .grok
echo 'GOOGLE_CLOUD_PROJECT="your-project-id"' >> .grok/.env
```
