...
- **Q: Where are the Gemini CLI configuration or settings files stored?**
  - A: The Gemini CLI configuration is stored in two `settings.json` files:
    1. In your home directory: `~/.grok/settings.json`.
    2. In your project's root directory: `./.grok/settings.json`.
...
- **DEBUG mode not working from project .env file**
  ...
  - **Solution:** Use a `.grok/.env` file instead, or configure the `excludedProjectEnvVars` setting in your `settings.json` to exclude fewer variables.
